...
//control while playing
  ...
  if((time.minutes >= alarm_minut) && (time.minutes < end_minut) && (time.Hours >= alarm_hour) && (time.Hours <= end_hour) && (time.weekday != exclusion_day))//указать время запуска специальной программы
  {
    musicPlayer.startPlayingFile("alarm.mp3");
    sec = 0;
    minut = 0;
