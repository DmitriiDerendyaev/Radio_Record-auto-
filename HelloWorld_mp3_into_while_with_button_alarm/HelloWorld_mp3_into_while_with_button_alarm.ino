...
*/
  ...
  //таймер срабатывания
  if((time.minutes >= beg_minut) && (time.minutes <= end_minut) && (time.Hours >= beg_hour) && (time.Hours <= end_hour))//указать время запуска основной программы
  {
  ...
  */
  ...
  if((time.minutes >= alarm_minut) && (time.minutes <= end_minut) && (time.Hours >= alarm_hour) && (time.Hours <= end_hour))//указать время запуска основной программы
  {
    musicPlayer.startPlayingFile("alarm.mp3");
    sec = 0;
    minut = 0;
